...
- [x] 在结构化 draw/discard/exhaust pile card 条目输出 `card_instance_id`。
...
- [x] 添加可重复 Steam 实机采样：`card_2` 从手牌 -> 弃牌 -> 抽牌堆 -> 手牌始终保持同一 ID；`card_4`（`ASCENDERS_BANE`）从手牌 -> 穷尽堆保持同一 ID；击杀 `enemy_1` 后，以旧 target ID 出牌返回 HTTP 409 `stale_target_instance_id`。（2026-07-13，STS2 v0.107.1。）

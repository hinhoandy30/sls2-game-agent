...
### Stable LLM Action Plan 的边界

当 COMBAT state 提供 card/enemy instance ID 时，CLI 默认让 LLM 返回有限的 `legal_action_id`
列表。Runtime 每执行一个动作都会 fresh-read/wait、重新验证下一个动作；一旦实体、target 或
screen 改变导致验证失败，它以 `validation_stopped:*` 结束余下计划。`--single-action` 用于旧
bridge 或排障。

这不是已解决的战术搜索系统：抽牌、击杀、生成牌、随机效果或选择界面出现时，Runtime 停止计划并
回到外层 loop 重新调用 Policy。需要保证复杂连招时，后续仍应新增 tactical solver，并以每一步的
最新 state 重新解析 card/enemy instance。

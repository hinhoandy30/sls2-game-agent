...
Runtime 的原始日志以审计优先，默认会比较长：

- `trajectory.jsonl`
  - 每一行是一个 step 的原始记录，包含决策前状态摘要、模型决策、执行结果、错误、耗时、
    token usage、segment 与 state hash。
- `segments.jsonl`
  - 记录 checkpoint / continue_run 后产生的时间线分支。
- `summary.json`
  - 正常结束时写入 run 级别结果、总耗时、错误数和 token 汇总。

平时复盘不建议直接读完整 `trajectory.jsonl`。先生成面向人的决策摘要：

```bash
uv run python -m sts2_agent_runtime.summarize_run \
  runs/<timestamp>_<run-id> \
  --output runs/<timestamp>_<run-id>/decision_summary.md
```

如果要给 Evaluation 或 notebook 做二次分析，可以生成结构化 JSON：

```bash
uv run python -m sts2_agent_runtime.summarize_run \
  runs/<timestamp>_<run-id> \
  --format json \
  --output runs/<timestamp>_<run-id>/decision_summary.json
```

`decision_summary.md` 会保留最核心的上下文：

- seed / run id，方便复现同一局；
- run 结果、step 数、总 token；
- 按 specialist agent 拆分的 token；
- 每一步的 screen、floor、HP、agent、action、reason；
- MAP 决策的 `route_groups`，用于看模型是否真的按全图路线规划；
- BUNDLE_SELECTION 决策的卡包内容，避免只看到“选了 option 0/1”却不知道模型当时应比较什么；
- COMBAT 决策的 `combat_audit`，用于看本回合是斩杀、满防、接受战损还是不可避免掉血；
- LLM 模型名、单步 token 和耗时；
- 发生错误的 step。

这个摘要是复盘入口；需要追具体字段或调 bug 时，再回到同目录的 `trajectory.jsonl`。
